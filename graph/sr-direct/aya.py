...
def extract_translated_text(response: str) -> str:
    response = response.split("<|START_OF_TURN_TOKEN|><|CHATBOT_TOKEN|>", 1)[1].strip().splitlines()[0]
    return response
...
def main(target_language: str):
    ...
    for n_shots in shot_configs:
        run(f'aya_{n_shots}_shot.txt', n_shots, eng_lines, None, tgt_lines, None, target_language, directory)
