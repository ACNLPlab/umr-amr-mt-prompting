...
def extract_translated_text(response: str, target_language: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if stripped.startswith(f"{target_language}:"):
            output = [line.strip() for line in lines[i:] if line.strip()]
            output[0] = output[0].replace(f"{target_language}:", "", 1).strip()
...
def build_prompt(examples, representation_type, n_shots, target_language):    
    ...
    if n_shots > 0:
        prompt += f"\n\nYour task is to translate the English sentence into {target_language} using the {representation_type} description."
        prompt += "\n\nExamples:"
    ...
    prompt += f"\n\nTranslate the following English sentence into {target_language}. Please output ONLY the translation:\n"
...
def main(target_language: str):
    ...
    for n_shots in shot_configs:
        run(f'mistral_{n_shots}_shot_umr.txt', n_shots, eng_lines, umr_descriptions, tgt_lines, 'Uniform Meaning Representation', target_language, directory)
        run(f'mistral_{n_shots}_shot_amr.txt', n_shots, eng_lines, amr_descriptions, tgt_lines, 'Abstract Meaning Representation', target_language, directory)
