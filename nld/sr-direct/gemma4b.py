...
token = "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"
...
def main(target_language: str):
    directory = f'results/gemma_4b/{target_language.lower()}'
    ...
    for n_shots in shot_configs:
        run(f'gemma4b_{n_shots}_shot_umr.txt', n_shots, eng_lines, umr_descriptions, tgt_lines, 'Uniform Meaning Representation', target_language, directory)
        run(f'gemma4b_{n_shots}_shot_amr.txt', n_shots, eng_lines, amr_descriptions, tgt_lines, 'Abstract Meaning Representation', target_language, directory)
